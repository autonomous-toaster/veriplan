...
Integrate steve as a prose-guidance dependency and wire its findings into the convertibility report as rephrase directives.

## Task Reference

| T ID | Description |
|------|-------------|
| T1.1 | add steve's exclusion-range builder method |
| T1.2 | add steve's configurable max-sentence-length builder method |
| T3.4 | use it for provenance-preserving scoping |
| T4.1 | build a curated `Ste` per artifact |
## Requirements
