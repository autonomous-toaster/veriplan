...
Implements the LSP stdio transport layer, workspace state management, file-to-change resolution, and ensures that SPIN model checking is never invoked during LSP events.
...
| T2.1 | Maintain in-memory ChangeStore |
| T2.2 | Resolve files to change directories |
