...
M1.1 SHALL correlate steve prose-guidance findings with grounding outcomes per requirement. When a requirement has both a steve style finding (PassiveVoice, PronounAmbiguity, or Hedging) AND a grounding outcome that is `Ungroundable` or `Ambiguous`, the convertibility report SHALL emit a single combined rephrase directive for that requirement instead of separate ones.
...
- **WHEN** M1.1 correlates the findings
...
M1.2 SHALL run the prose/grounding correlation in all StrictnessProfile modes, including `Lax`. In `Lax` mode, when a requirement is `Ungroundable` (normally info-only) and also has a steve style finding, the combined directive SHALL still be emitted at info severity.
...
- **WHEN** M1.2 runs the correlation
