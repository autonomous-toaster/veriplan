...
fn test_truncate() {
    ...
    let t = crate::util::truncate("a very long string that should be truncated", 20);
...
fn test_display_label_fixed_time_no_ordering() {
    let c = TranslatedConstraint {
        ...
        ltl: Some(LtlFormula::Always(LtlCondition::Eventually(Box::new(
            LtlCondition::Atom("done_t1_1".into()),
        )))),
        strength: Rfc2119Strength::Must,
        is_hard: true,
    };
